// /assets/js/xp.config.js  — single source of truth

export const XP_CONFIG = Object.freeze({
  foundry4: {
    tierCaps: { T0: 1200, T1: 1400, T2: 1600, T3: 2200, T4: 2400 },
    monthly: {
      attendance: 200,
      attendanceNegFishEventsMax: 4,
      arena: { T0: 40, T1: 60, T2: 60, T3: 80, T4: 80 },
  ...
  // ✅ Youth caps start at 800 (+200), Hero unchanged
  foundry8: {
    tierCaps: { T0: 800, T1: 1000, T2: 1200, T3: 1400, T4: 1600, T5: 1800, T6: 2000, T7: 2200, T8: 2400 },
    monthly: {
      attendance: 160,
      attendanceNegFishEventsMax: 4,
      arena: { T0: 0, T1: 40, T2: 40, T3: 60, T4: 60, T5: 60, T6: 60, T7: 60, T8: 80 },
      character: { parentRewardMax: 10, parentDeductMax: 10, netMax: 10 }
    },
    arena: { battle: 10, podium: 5, styleIQ: 5, styleIQMaxPerEvent: 1 },
    prestigeYearly: { state: 3, regional: 1, national: 1 },
    ranks: { T0:"Shadow", T1:"Recruit", T2:"Combatant", T3:"Competitor", T4:"Warrior", T5:"Champion", T6:"Veteran", T7:"Sandman", T8:"Hero" }
  }
});
...
export function getCap(trackBaseOrKey, tier = "T0") {
  ...
  return (base === "F8" ? XP_CONFIG.foundry8.tierCaps[tier] : XP_CONFIG.foundry4.tierCaps[tier]) ?? 800;
}
export function getRank(trackBaseOrKey, tier = "T0") {
  const base = normalizeBase(trackBaseOrKey);
  return (base === "F8" ? XP_CONFIG.foundry8.ranks[tier] : XP_CONFIG.foundry4.ranks[tier])

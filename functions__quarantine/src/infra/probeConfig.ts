...
export function installConfigProbe() {
  // eslint-disable-next-line @typescript-eslint/no-var-requires
  const Module = require("module");

  const origLoad = Module._load;

  function maybeWrapConfig(loaded: any, request: string, parent: any) {
    if (!loaded || typeof loaded !== "object" && typeof loaded !== "function") return loaded;

    // If the module itself has config()
    if (typeof (loaded as any).config === "function") {
      const original = (loaded as any).config;
      (loaded as any).config = (...args: any[]) => {

...
const ACTIVE_TRACK = "f8";
const ACTIVE_SEGMENT_ID = "segment1";
const VAULT_PATH = "/vault/conditioning/f8/segment1/sessions.json";
...
function fatalMissingId() {
  document.body.innerHTML = `
    <main style="max-width:900px;margin:40px auto;padding:16px;font-family:system-ui;color:#e8eef7">
      ...
      <p style="margin:0 0 14px;color:#9ca3af">This page needs <code>?id=F8_0001</code>.</p>
    </main>
  `;
...
function normalizeTierValue(athlete = {}) {
  const raw =
    athlete?.tierLevel ??
    athlete?.tierIndex ??
    athlete?.tierN ??
  ...
  if (/^T\d+$/.test(str)) return Number(str.slice(1));
...
function getRemoteLimitByTier(tier) {
  if (tier <= 2) return 0;
  if (tier <= 4) return 1;
  return 2;
}
...
function getRemoteUsageThisMonth(submissions = {}) {
  const monthKey = getCurrentMonthKey();
  const assignedAtMonthPrefix = monthKey;
  ...
  if (
    assignment &&
    ["assigned", "completed"].includes(String(assignment?.status || "").toLowerCase())
  ) {
    const ts = assignment?.assignedAt;
    const iso = ts?.toDate?.()?.toISOString?.() || "";
    if (iso.startsWith(assignedAtMonthPrefix)) count += 1;
  }
...
async function loadConditioningSession() {
  ...
  if (!isFoundry8(athleteId)) {
    container.innerHTML = `
      <div class="lane-card">
        This page is for Foundry 8 athletes only.
      </div>
    `;
    return;
  }
  ...
  try {
    ...
    const remoteUsedThisMonth = getRemoteUsageThisMonth(submissions);
    ...
    if (assignmentStatus === "assigned" && assignedPreset) {
      ...
      if (remoteLimit <= 0) {
        container.innerHTML = `
          <div class="lane-card">
            <div style="font-weight:900;font-size:1.05rem;">Conditioning Locked</div>
            <div style="margin-top:.45rem;opacity:.8;">
              Conditioning does not open yet at this level.
            </div>
          </div>
        `;
        return;
      ...
      const rawNextSessionN = completed.length ? Number(completed[0].sessionN || 0) + 1 : 1;
      const cappedNextSessionN = Math.min(rawNextSessionN, sessions.length);

      session =
        sessions.find((s) => Number(s?.n) === cappedNextSessionN) ||
    ...
    container.innerHTML = `
      <div class="lane-card">
        ...
        <div style="margin-top:.45rem;opacity:.78;font-size:.9rem;">
          Tier ${esc(String(tier))} · Remote Used This Month: ${esc(String(remoteUsedThisMonth))} / ${esc(String(remoteLimit))}
        </div>
    ...
    submitBtn?.addEventListener("click", async () => {
      ...
      try {
        ...
        console.error("F8 conditioning submit error:", err);
        if (statusEl) statusEl.textContent = "Error submitting (see console).";
        submitBtn.disabled = false;
      }
    });
  } catch (err) {
    console.error("F8 conditioning load error:", err);

...
// RANK CONFIG
///////////////////////
let RANK_CONFIG = null;

async function loadRankConfig() {
  const res = await fetch('/lab/timer-engine/data/combat_rank_timer.json');
  if (!res.ok) {
    throw new Error(`Failed to load rank config: ${res.status}`);
  }
  RANK_CONFIG = await res.json();
  console.log('RANK_CONFIG loaded', RANK_CONFIG);
}
...
async function init(){
  ...
  conceptEl.addEventListener('change', ()=>{
    sessionStorage.setItem(key, JSON.stringify({concept:conceptEl.value,preset:presetEl.value}));
    loadSelection();
  });

...
const scoreEl  = document.getElementById("courage-score");
const valEl    = document.getElementById("courage-val");
...
async function boot() {
  try {
    ...
    // If Step 6 already locked, forward immediately
    if (athlete?.onboarding?.locks?.step6 === true) {
      window.location.href = `/athlete-onboarding/step-7.html?id=${encodeURIComponent(uid)}`;
      return;
    }
...
nextBtn && (nextBtn.onclick = async () => {
  ...
  try {
    ...
    await updateDoc(doc(db, "athletes", uid), {
      ...
      // Step 6 completed → next step is 7
      "onboarding.step": 7,

      "onboarding.selfAssess.courage": v,
      "onboarding.selfAssess.courageAt": serverTimestamp(),

      // Step-lock model
      "onboarding.locks.step6": true,

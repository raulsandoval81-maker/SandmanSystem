...
async function start() {
  ...
  targets.forEach((target) => {
    ...
    const stateDetail = live.row
      ? `<span class="schedule-live__class">${esc(spanish ? (live.row.titleEs || live.row.title) : live.row.title)}</span><span class="schedule-live__time">${esc(live.row.label || live.row.time || "")}</span>${live.state === "next" ? `<span class="schedule-live__countdown">${spanish ? "Comienza en" : "Starts in"} ${live.minutesUntil} min</span>` : ""}`
      : later
        ? `<span class="schedule-live__class">${spanish ? "Próxima" : "Next"}: ${esc(spanish ? (later.row.titleEs || later.row.title) : later.row.title)}</span><span class="schedule-live__time">${esc(later.day)} ${esc(later.row.label || later.row.time || "")}</span>`
        : `<span class="schedule-live__empty">${spanish ? "No hay una clase programada hoy." : "No class is scheduled today."}</span>`;
    target.innerHTML = `<article class="schedule-card schedule-card--today ${stateClass}"><h2>${stateTitle}</h2><div class="schedule-live__detail">${stateDetail}</div></article>${[...groupByDay(expandScheduleRowsByDay(schedule.weekly))].map(([day, rows]) => `<article class="schedule-card schedule-card--day"><h2>${esc(spanish ? (rows[0]?.dayEs || day) : day)}</h2>${rows.map((row) => `<div class="schedule-session schedule-category--${esc(row.category || "unknown")} schedule-provider--${esc(row.provider || "unknown")}"><h3 class="schedule-session__title">${esc(spanish ? (row.titleEs || row.title) : row.title)}</h3><p class="schedule-details">${esc(row.details ? (spanish ? (row.detailsEs || row.details) : row.details) : "")}</p><p class="schedule-time">${esc(row.label || row.time || `${row.start || ""}–${row.end || ""}`)}</p><p class="schedule-meta">${esc(scheduleCategoryLabel(row))} • ${esc(scheduleProviderLabel(row))} • ${esc(row.instructor || "Instructor TBA")}</p></div>`).join("")}</article>`).join("")}`;
  });

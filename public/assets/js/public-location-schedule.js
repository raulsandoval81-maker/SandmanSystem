...
async function start() {
  ...
  targets.forEach((target) => {
    ...
    target.innerHTML = `<article class="schedule-card schedule-card--today ${stateClass}"><h2>${stateTitle}</h2><p>${stateDetail}</p></article>${[...groupByDay(expandScheduleRowsByDay(schedule.weekly))].map(([day, rows]) => `<article class="schedule-card schedule-card--day"><h2>${esc(spanish ? (rows[0]?.dayEs || day) : day)}</h2>${rows.map((row) => `<div class="schedule-session schedule-category--${esc(row.category || "unknown")} schedule-provider--${esc(row.provider || "unknown")}"><h3 class="schedule-session__title">${esc(spanish ? (row.titleEs || row.title) : row.title)}</h3>${row.details ? `<p>${esc(spanish ? (row.detailsEs || row.details) : row.details)}</p>` : ""}<p class="schedule-time">${esc(row.label || row.time || `${row.start || ""}–${row.end || ""}`)}</p><p>${esc(scheduleCategoryLabel(row))} • ${esc(scheduleProviderLabel(row))} • ${esc(row.instructor || "Instructor TBA")}</p></div>`).join("")}</article>`).join("")}`;
  });

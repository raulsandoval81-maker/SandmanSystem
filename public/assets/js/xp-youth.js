...
/**
 * Current youth UI rule:
 * Shadow = 3 stripes
 * Others = 4 stripes
 *
 * This is intentionally local UI behavior for now.
 */
...
export function updateYouthBar(totalXP = 0) {
  ...
  const tierKey = String(curName || "").toLowerCase();

  // ✅ Use shared service as source of truth
  const pct = clamp(Number(info?.percent || 0));
  const capXP = Math.max(1, Number(info?.capXP || 1));
  ...
  // derive start/cap safely for dataset/debugging only
  const xpInTier = Math.max(0, Number(info?.xpInTier || 0));
  const start = Math.max(0, Number(totalXP) - xpInTier);
  const cap = start + capXP;
  ...
  rankBar.dataset.excessXp = String(Math.max(0, Number(totalXP) - cap));
  ...
  // status line
  if (sLineEl) {
    if (isFinal) sLineEl.textContent = "HERO - MAXED";

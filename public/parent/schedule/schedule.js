...
function render(schedule, athlete) {
  ...
  todayCard.innerHTML = `<div class="status-row"><div class="k">${stateTitle}</div><div class="v">${stateDetail}</div></div>${live.todayRows.length ? `<div class="weekly-list">${live.todayRows.map((row) => `<div class="item schedule-category--${esc(row.category || "unknown")} schedule-provider--${esc(row.provider || "unknown")}"><div class="item-top"><div class="item-day">${esc(row.title)}</div><div class="item-sub">${esc(displayTime(row))}</div></div><div class="item-sub">${esc(row.details || "")}</div><div class="item-sub">${esc(meta(row))}</div></div>`).join("")}</div>` : ""}`;

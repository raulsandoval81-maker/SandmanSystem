...
btnPost?.addEventListener("click", async () => {
  ...
  try {
    await addDoc(collection(db, "paraAnnouncements"), {
      ...
      archived: false,
      deleted: false,
      audienceType,          // all | parents | athletes (coaches later if you want)
...
function renderFeed() {
  ...
  const visible = annItems.filter(
    (x) => x.teamId === TEAM_ID && x.archived !== true && x.deleted !== true
  );
  ...
  // Bind actions (rebind every render; simple + V1 safe)
...
onSnapshot(
  ...
  (err) => {
    ...
    if (feedEl) feedEl.innerHTML = `<div style="opacity:.75;padding:10px 0;">Error loading broadcasts.</div>`;
  }
);
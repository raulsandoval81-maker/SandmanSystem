...
// ✅ Single import: always pull from your central bootstrap
import {
  db, doc, setDoc, getDoc, updateDoc, addDoc, collection,
...
Object.assign(window, { db, doc, setDoc, getDoc, updateDoc, addDoc, collection, getDocs, query, where, orderBy, limit });
// --- YOUR CONFIG ---
// ========================= FIREBASE IMPORTS =========================
...
// Master virtue map — system official sets
...
const F8_CODES = ["FC","EF","AT","RS","SP","AG","PW","CB"];
const F4_CODES = ["IN","PT","DS","HN","ST","WS","CR","TN"];
...
const XP_CONFIG = {
  F8: {
    caps:  { T0: 800, T1: 1000, T2: 1200, T3: 1400, T4: 1600, T5: 1800, T6: 2000, T7: 2200, T8: 2400 },
...
await refreshApproved?.();
    ...
    await setDoc(doc(db, "athletes", uid), {
      ...
      rankColor: tierMeta?.rankColor || "white",
      rankName: tierMeta?.rankName || "Shadow",
      padlock, virtueCode, virtueName,
      xp: 0,
      xpCap: XP_CONFIG?.F8?.T0 || 600, // or directly: xpCap: 600
      stripeCount: 0,
      cooldownUntil: null,

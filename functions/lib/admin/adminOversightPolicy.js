...
exports.ADMIN_OVERSIGHT_LOCATIONS = void 0;
exports.buildAdminOversightSummary = buildAdminOversightSummary;
const staffAuthorization_1 = require("../services/staffAuthorization");
exports.ADMIN_OVERSIGHT_LOCATIONS = Object.freeze([
...
function locationIdOf(data, fallback = "") {
    return clean(data.locationId ||
...
function accessMode(data) {
    const explicit = lower(data.access?.mode);
    if (["parent_managed", "hybrid", "self_managed"].includes(explicit))
        return explicit;
    return clean(data.authUid) ? "unclassified" : "parent_managed";
}
function baseLocation(locationId, locationName) {
    return {
        locationId,
        locationName,
        ...
        schedule: { status: "missing", publishedAt: null, updatedAt: null },
        staffing: { management: [], coaches: [] },
...
function buildAdminOversightSummary(sources, now = Date.now()) {
    const locations = new Map(exports.ADMIN_OVERSIGHT_LOCATIONS.map(({ locationId, locationName }) => [locationId, baseLocation(locationId, locationName)]));
    const unknownBySource = {};
    const unknownRecords = new Set();
    const markUnknown = (source, recordId) => {
        const key = `${source}:${recordId}`;
        if (unknownRecords.has(key))
            return;
        unknownRecords.add(key);
        unknownBySource[source] = (unknownBySource[source] || 0) + 1;
    };
    const resolve = (source, record, fallback = "") => {
        const locationId = locationIdOf(record.data, fallback);
        const location = locations.get(locationId);
        if (!location)
            markUnknown(source, record.id);
        return location;
    };
    for (const record of sources.messages) {
        const location = resolve("messages", record);
        if (!location || isTerminal(record.data))
            continue;
        location.inbox.pending += 1;
        location.inbox.totalOpen += 1;
        if (!clean(record.data.assignedManagerUid))
            location.inbox.unassigned += 1;
        const escalated = ["ESCALATED", "ADMIN_REVIEW", "PENDING_ADMIN"].includes(upper(record.data.routingStage || record.data.assignmentStatus)) || ["urgent", "high"].includes(lower(record.data.priority));
        if (escalated)
            location.inbox.escalated += 1;
    }
    const countPipeline = (source, field) => {
        for (const record of sources[source]) {
            const location = resolve(String(source), record);
            if (location && !isTerminal(record.data))
                location.pipeline[field] += 1;
        }
    };
    ...
    for (const record of sources.athletes) {
        ...
        if (mode in location.members)
            location.members[mode] += 1;
    }
    ...
    for (const record of sources.practices) {
        ...
        if (location && lower(record.data.status) === "active")
            location.attendance.openPractices += 1;
    }
    for (const record of sources.attendance) {
        ...
        if (status === "pending_review")
            location.attendance.pendingReview += 1;
        if (status === "finalized") {
            location.attendance.recentFinalized += 1;
            if (isToday(record.data.finalizedAt, now))
                location.attendance.finalizedToday += 1;
        }
    }
    ...
    for (const record of sources.staff) {
        const staff = (0, staffAuthorization_1.normalizeStaffRecord)(record.data);
        if (staff.status !== "active" || !["management", "coach"].includes(staff.role))
            continue;
        const identity = {
            uid: record.id,
        ...
        if (!staff.scope.locationIds.length)

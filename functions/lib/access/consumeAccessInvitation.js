...
function invitationError(error) {
    ...
    if (reason === "DIFFERENT_ATHLETE_UID" || reason === "CALLER_ALREADY_BOUND") {
        throw new https_1.HttpsError("failed-precondition", "This Athlete access conflicts with an existing account binding.");
    }
...
exports.consumeAccessInvitation = (0, https_1.onCall)(async (req) => {
    ...
    if (!authEmail || req.auth.token.firebase?.sign_in_provider === "anonymous") {
        throw new https_1.HttpsError("permission-denied", "An email-backed account is required.");
    }
    ...
    return db.runTransaction(async (tx) => {
        ...
        if (String(invitation.role || "") === "athlete") {
            ...
            if (!athleteSnap.exists)
                throw new https_1.HttpsError("failed-precondition", "Athlete record is unavailable.");
            const athlete = athleteSnap.data() || {};
            const existingBindings = await tx.get(db.collection("athletes").where("authUid", "==", callerUid).limit(2));
            let decision;
            try {
                decision = (0, accessInvitationPolicy_1.assertConsumableAthleteInvitation)({
                    exists: true,

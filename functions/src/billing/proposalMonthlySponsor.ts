...
import type { LockedRecurringPricing } from "../proposals/lockedRecurringPricing";

export async function ensureProposalMonthlySponsorCoupon(
  stripe: Stripe,
  proposalId: string,
  pricingVersion: string,
  amounts: LockedRecurringPricing
): Promise<Stripe.Coupon | null> {
  if (amounts.monthlySponsorCents === 0) return null;
  ...
  try {
    ...
    try {
      ...
      if ((createError as { code?: string }).code !== "resource_already_exists") throw createError;
      coupon = await stripe.coupons.retrieve(id);
    }
  }

  if (
    coupon.id !== id || coupon.valid !== true ||
...
export function verifyProposalSubscriptionDiscount(
  ...
  const actualBaseCents = subscription.items.data.reduce((total, item) => {
    ...
    if (
      price.currency !== "usd" || price.unit_amount === null ||
...
export interface ProposalSubscriptionIdentity {
  proposalId: string;
  checkoutSessionId: string;
  customerId: string;
  firstRecurringChargeUnix: number;
...
export function verifyProposalSubscription(
  subscription: Stripe.Subscription,
  identity: ProposalSubscriptionIdentity
): string | null {
  ...
  const byPrice = (a: { price: string }) => a.price;
...
export async function recoverOrCreateProposalSubscription(
  stripe: Stripe,
  identity: ProposalSubscriptionIdentity,
  savedSubscriptionId: string | null,
  create: () => Promise<Stripe.Subscription>
): Promise<{ subscription: Stripe.Subscription; discountId: string | null; recovered: boolean }> {
  ...
  if (savedSubscriptionId) {
    ...
    do {
      ...
      matches.push(...page.data.filter((candidate) =>
        candidate.metadata?.proposalId === identity.proposalId
      ));
  ...
  if (!subscription) subscription = await create();

...
export type AddDisciplineResult = {
  uid: string;
  discipline: string;
  receiptId?: string;
  idempotent: boolean;
...
export async function addDisciplineToAthlete(
  db: Firestore,
  coachUid: string,
  input: AddDisciplineInput
): Promise<AddDisciplineResult> {
  const existingAthleteUid =
    String(input.existingAthleteUid || "")
      .trim()
      .toUpperCase();
  ...
  const trackCode =
    clean(input.trackCode);

  const programTrack =
    clean(input.programTrack);

  const framework =
    clean(input.framework);

  const foundry =
    clean(input.foundry);

  const ladderKey =
    String(input.ladderKey || "")
      .trim();

  const rosterIds =
    Array.isArray(input.rosterIds)
      ? input.rosterIds
          .map((x) => String(x || "").trim())
          .filter(Boolean)
      : [];
  ...
  if (!trackCode || !programTrack) {
    throw new HttpsError(
      "invalid-argument",
      "Missing trackCode or programTrack."
    );
  }
  ...
  return db.runTransaction(
    async (tx) => {
      ...
      if (intakeRef) {
        ...
        const status =
          clean(intakeData.status);

        const approvedUid =
          String(
            intakeData.approvedUid || ""
          )
            .trim()
            .toUpperCase();
        ...
        if (
          ...
          throw new HttpsError(
            "failed-precondition",
            `Submission not approvable: ${status}`
          );
        }
      }
      ...
      if (alreadyNested || alreadyLegacy) {
        throw new HttpsError(
          "already-exists",
          `${existingAthleteUid} already has ${discipline}.`
        );
      }
